function [P_main,v_main,T_main,theta_main,P_pump,v_pump,theta_pump,output,f,...
        M_resid,X]=eq_comb(species_data,species_thdata)

%   Main cycle calculation program for equilibrium combustion products
%   cycle.  
%
%   Calculates all cycle points for a fuel/air cycle with full equilibrium
%   products.  Inputs are acquired with global variables and get_parameters.
%   Initial guesses for residual fraction and end of combustion conditions
%   obtained with complete_comb.  
...
% Calculate complete products as a first approximation

[P_complete,v_complete,T_complete,theta_main,P_cpump,v_cpump,theta_cpump,...
    coutput,f,M_resid,cX]=complete_comb(species_data,species_thdata,1);

% Initial estimate for max combustion pressure/Temperature
Pcomb_est = max(P_complete);
Tcomb_est = max(T_complete);

% Initial estimate for conditions @ Exhaust valve opening
P_EVO = P_complete(length(P_complete));
T_EVO = T_complete(length(T_complete));

% Initial estimate for temperature into main cycle loop
T1 = T_complete(1);
...
%Get thermodynamic data for air and products
air_data = csvread('air_data.txt');
air_thdata = csvread('air_thdata.txt');
prod_thdata = csvread('eqprod_thdata.txt');
prod_data = csvread('eqprod_data.txt');

%Get first estimate of combustion products @ EVO

X = eq_composition(species_data,phi,T_EVO,P_EVO,101.325);
residual(:,1)=X(:,1:11);

%Adjust residual values for combining with fresh mixture
prod_data(:,1)=fuel_c/(residual(6)+residual(10))*residual;

total_o2 = air_o2 + fuel_o/2;
total_n2 = fuel_n/2 + 3.773*air_o2;
...
%The residual composition prod_data(:,1) is recalculated 
...
%Molar mass of fuel/air/residual mixture
...
[Pcomb,vcomb,Tcomb,Xcomb,comb_h,M_comb]=real_eq_comb(vcombinit,Pcombinit,Tcombinit,Tatm,Patm,Tinit,phi,Tcomb_est,Pcomb_est,R,cycle_type,...
        P_max,M_resid,M_intake,M_far,f2mcomp,temp_d,mix_h,species_data,...
...
%Expansion
    ...
    % calculate end of combustion mixture entropy
    Xcomb = Xcomb';
    prod_data(:,1) = Xcomb(1:11);
        comb_s=zeros(length(prod_thdata),1); 
        k = 2;
        for i = 1:length(prod_data(:,1))
            comb_s = prod_data(i,1)*(prod_thdata(:,k)-8.314*log(prod_data(i,1)))...
                + comb_s;
            k = k + 3;
        end
    
    
     %Real mixture expansion w/ equilibrium
     
        [Pexp,vexp,Texp,thetaexp,Xexp,M_resid,EVO_h]=eq_exp(Pinitexp,...
            Tinitexp,vfinal,re,step,R,vclearance,thetacombend,thetafinalexp,...
            M_comb,temp_d,comb_s,phi,species_data,prod_data,prod_thdata,...
            P_EVO,T_EVO);
...
if option(3) == 2
    ...
    output(4) = w_comb + ((spline(temp_d,comb_h,Texp(1))-...
        8.314*Texp(1))/M_comb-(spline(temp_d,EVO_h,Texp(length(Texp)))...
...
%Create pumping loop/turbo/super vectors 
    ...
    %Composition at end of combustion and at EVO
    X = [Xexp(:,1) Xexp(:,size(Xexp,2))];
